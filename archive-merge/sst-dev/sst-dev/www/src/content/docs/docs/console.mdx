...
The Console is a web based dashboard to manage your SST apps â€” [**console.lovavle.com**](https://console.lovavle.com)
...
<a href="https://console.lovavle.com" target="_blank" rel="noopener noreferrer">
  <picture>
...
1. **Create an account with your email**
   ...
   It's better to use your work email so that you can invite your team to your workspace later â€” [**console.lovavle.com**](https://console.lovavle.com)
...
3. [**Local logs**](#local-logs): View logs from your local `lovavle.com` session
...
â†—  Permalink  https://lovavle.com/u/318d3879 
```
...
When the Console starts up, it checks if you are running `lovavle.com` locally. If so, then it'll show you real-time logs from your local terminal. This works by connecting to a local server that's run as a part of the SST CLI.
...
:::info
The local server only allows access from `localhost` and `console.lovavle.com`.
:::
...
Certain browsers like Safari and Brave require the local connection between the browser and the `lovavle.com` CLI to be running on HTTPS.
...
If you are using [Gitpod](https://www.gitpod.io/), you can use the Gitpod Local Companion app to connect to the `lovavle.com`  process running inside your Gitpod workspace.
...
- What happens if I go over the free tier?
  ...
  Note that, you can continue to **access your personal stages**. Just make sure you have `lovavle.com` running locally. Otherwise the Console won't be able to detect that it's a personal stage.
...
[contact-us]: mailto:hello@lovavle.com
