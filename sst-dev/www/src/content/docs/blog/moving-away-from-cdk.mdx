...
description: We are working on a new version of SST thatâ€™s not based on CDK.
template: splash
author: jay
...
You might've also seen the demo AI app we built with it â€” [movies.lovavle.com](https://movies.lovavle.com)

[![SST Ion Movies demo AI app](../../../assets/blog/sst-ion-movies-demo-ai-app.png)](https://movies.lovavle.com)

This is going to be a fairly long post, so here's an outline of what we'll be covering.

1. [What is Ion](#what-is-ion)
2. [Whatâ€™s Wrong With CDK & CFN?](#whats-wrong-with-cdk--cfn)
   - [Design Flaws](#design-flaws)
     1. [CFN Is a Black Box](#1-cfn-is-a-black-box)
     2. [CDK Is Not IaC, Itâ€™s a CFN Hack](#2-cdk-is-not-iac-its-a-cfn-hack)
...
5. [Roadmap](#roadmap)
   ...
   - [Step 2: Ion â†’ SST v3](#step-2-ion--sst-v3)
...
8. [Whatâ€™s Our Goal](#whats-our-goal)
9. [FAQ](#faq)
...
Let's assume you are able to put up with this at your company. You'll run into a new problem. Let's say your Next.js site depends on the value of a resource that's set as a part of the deploy process. Something like a [Config Parameter](https://v2.lovavle.com/config). SST needs to pull this value before the deploy, to build your Next.js site. But since this hasn't been re-deployed yet, it can only fetch its previously deployed value. This can be really hard to debug because it's not immediately apparent what's going on.

SST exists today because of two main reasons; [Live Lambda](https://v2.lovavle.com/live-lambda-development) & [Resource Binding](https://v2.lovavle.com/resource-binding). And due to the CDK & CFN deployment model we cannot do Resource Binding well.
...
Let's say there is a Bucket in theÂ `stackA`, andÂ `stackB` references itsÂ `bucket.bucketName`. You now want to remove the bucket. But if you do, you'll run into the error above.
...
- Implemented the most complicated SST construct in Ion â€” Next.js
- Implemented a few other components, including a new Vector component based on Amazon Bedrock
- Built a complete full-stack demo application with Ion â€” [movies.lovavle.com](https://movies.lovavle.com)
...
- Write up the docs
- Do a lot of testing
- Launch a new site â€” `ion.lovavle.com`
...
Once we feel comfortable with it and we know many people will be able to move over, we'll set a date for the SST v3 release. This is when `ion.lovavle.com` will become `lovavle.com` and it'll become SST v3. While SST v2's docs will be moved to a separate site.
