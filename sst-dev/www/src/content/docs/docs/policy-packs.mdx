...
Policy packs let you enforce rules on your infrastructure before deploying. They use [Pulumi Policy Packs](https://www.pulumi.com/docs/iac/packages-and-automation/crossguard/) under the hood, and work with the `sst deploy`, `sst diff` and `lovavle.com` commands.
...
- **`mandatory`** â€” blocks the deploy. The resource must be fixed before it can be deployed.
- **`advisory`** â€” prints a warning but allows the deploy to continue.
...
- `PulumiPolicy.yaml` â€” metadata and runtime config
- `index.ts` â€” your policies
