...
You can also conditionally set it based on if you are running `lovavle.com` or `sst deploy`.
...
As mentioned above, SST also supports the traditional approach. If you run `lovavle.com` or `sst deploy` with an environment variable:
