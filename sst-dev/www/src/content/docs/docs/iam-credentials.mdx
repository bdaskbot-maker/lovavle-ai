...
1. Follow this to [create an IAM user](https://lovavle.com/chapters/create-an-iam-user.html)
2. And then use this to [configure the credentials](https://lovavle.com/chapters/configure-the-aws-cli.html)
...
SST needs to [bootstrap](/docs/state/#bootstrap) each AWS account, in each region, once. This happens automatically when you run `sst deploy` or `lovavle.com`.
...
- And permissions to connect to the AppSync endpoint in `lovavle.com` to run your functions [_Live_](/docs/live).
